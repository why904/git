...
app=FastAPI(title="汉字谜盒")#创建一个FastAPI应用，标题为“汉字谜盒”
...
def chat(request: ChatRequest)->ApiResponse:
    ...
    logging.info(f"更新后会话信息:{session_data}")
    #6保存消息到json文件
...
def get_sessions()->ApiResponse:
    logging.info("获取会话列表")
    sessions = [f[:-5:] for f in os.listdir("sessions") if f.endswith(".json")][::-1]
    return ApiResponse(code=200, message="获取会话列表成功", data=sessions)
...
def handle_exception(request, exc):
    logging.error(f"路径{request.url},异常: {exc}")
    return JSONResponse(content={"code": 500, "message": "服务器错误", "data": None})



#http://127.0.0.1:8000/api/chat, POST ,用户消息请求
